...
const ScrollMarqueeContext = React.createContext(null);

export function ScrollMarqueeContainer({ children, className, ...props }) {
  ...
  return (
    <ScrollMarqueeContext.Provider value={velocityFactor}>
      <div className={cn("relative w-full", className)} {...props}>
        {children}
      </div>
    </ScrollMarqueeContext.Provider>
  );
}

export function ScrollMarquee(props) {
  const sharedVelocityFactor = useContext(ScrollMarqueeContext);
  if (sharedVelocityFactor) {
    return <ScrollMarqueeImpl {...props} velocityFactor={sharedVelocityFactor} />;
  }
  return <ScrollMarqueeLocal {...props} />;
}

function ScrollMarqueeImpl({
  children,
  baseVelocity = 5,
  direction = 1,
  className,
...
function ScrollMarqueeLocal(props) {
  const { scrollY } = useScroll();
  ...
  return (
    <ScrollMarqueeImpl {...props} velocityFactor={localVelocityFactor} />
  );
}