...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }) {
  ...
  const { scene } = useGLTF('/localhub-card.glb');
  const texture = useTexture('/localhub.png');
  ...
  // Rope length configuration
  const ropeLength = 1.5; 
  ...
  // 🌟 FIX 1: Attach rope exactly to the top edge of the card's collider
  useSphericalJoint(j3, card, [
    [0, 0, 0],
    [0, 1.2, 0] // 1.1 units above the center of the card
  ]);
    ...
    <>
      <group position={[0, 4, 0]}>
        ...
        <RigidBody position={[0, -4, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          ...
          <group
            scale={2.25}
            // 🌟 FIX 2: Removed the -1.2 Y-offset. 
            // The visual card now sits perfectly inside its physics collider.
            position={[0, 0, -0.05]} 
